...
export const startAndGetSessionToken = async ({
  ...
  try {
    ...
    if (!data.idToken) {
      ...
      throw new Error(`Fail to start session.`);
    }
    ...
    if (
      ...
      ["INVALID_PASSWORD", "EMAIL_NOT_FOUND", "INVALID_LOGIN_CREDENTIALS"].some(
        message => error.message.includes(message),
      )
    ) {
      throw new Error(
        `Fail to start session. Your password might be incorrect. ${error.message}`,
      );
    }
    logger.error(`Fail to start session. ${error}`);
    throw error;
  }
};
